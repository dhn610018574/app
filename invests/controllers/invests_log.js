; !function () {
    ...
    dropDown();
    function getData() {
        var contractPrefix = GetQueryString('contractPrefix') + '';
        if (contractPrefix === 'null') {
            contractPrefix = ''
        } else {
            contractPrefix = GetQueryString('contractPrefix') + '';
        }
        // var contractPrefix = 'F3M-1-';
        $.ajax({
            url: path + '/webp2p_interface_mysql/investment/product/record?contractPrefix=' + contractPrefix + '&pageNumber=0&pageSize=20',
            type: 'get',
            dataType: 'json',
            data: {},
            success: function (data) {
                console.log(data.data.lsirr[0].investDate);
                if (data.result === '0000') {
                    
                    data.data.lsirr.forEach(function (value, index) {
                        if (data.data.lsirr[index].investDate) {
